...
class Road_Detection:
    def __init__(self, frame):
        ...
        self.symbol_size                = 50
        self.straight_symbol            = cv2.imread("lurus.png", cv2.IMREAD_UNCHANGED)
        self.turn_left_symbol           = cv2.imread("belok_kiri.png", cv2.IMREAD_UNCHANGED)
        self.turn_right_symbol          = cv2.imread("belok_kanan.png", cv2.IMREAD_UNCHANGED)
    ...
    def thresholdingProcess(self):
        ...
        # black_lane              = cv2.inRange(hsv_frame, (0, 130, 104), (255,255,255))
        # self.threshold_frame    = cv2.bitwise_or(white_lane, black_lane)
    ...
    def calculate_curvature(self, frame):
        ...
        cv2.rectangle(frame, (0, 0), (int((30/100)*self.width), int((30/100)*self.height)), (50, 50, 50), -1)
        ...
        if left_fit_cr[0] < 0 and right_fit_cr[0] < 0 and self.ave_curvem < 50:
            ...
            cv2.putText(frame,'Curvature = '+str(self.ave_curvem)[:7]+' m', (int(( 1/100)*self.width), 
...
def main():
    ...
    cap = cv2.VideoCapture('data/tolsda.mp4')
